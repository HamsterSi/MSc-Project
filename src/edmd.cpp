 ...
 * Return:    None
 */
void EDMD::calculate_NB_Forces(Tetrad* tetrad1, Tetrad* tetrad2) {
    ...
    for (i = 0 ; i < tetrad1->num_Atoms; i++) {
        tetrad1->NB_Forces[i] = 0.0;
    }
    tetrad2->energies[1] = tetrad2->energies[2] = 0.0;
    for (i = 0 ; i < tetrad2->num_Atoms; i++) {
        tetrad2->NB_Forces[i] = 0.0;
    }
    
    for (i = 0; i < tetrad1->num_Atoms; i++) {
        for (j = 0;  j < tetrad2->num_Atoms; j++) {
            
            dx = tetrad1->coordinates[3*i]   - tetrad2->coordinates[3*j];
            dy = tetrad1->coordinates[3*i+1] - tetrad2->coordinates[3*j+1];
            dz = tetrad1->coordinates[3*i+2] - tetrad2->coordinates[3*j+2];
